angular.module('appAllHouses',[]).controller('allHousesView',function ($scope,$http) {
    ...
    $scope.loadAllHouses = function () {
        $http.get(contextPath + '/loadAllActiveSensorStations')
    ...
    $scope.goToConcreteHouseSeite = function(sensorStationId){
        window.location.href = contextPath + "/user/concreteHouse.html?id="+sensorStationId;
    }

    $scope.check = function(id){
        $http.post(contextPath + '/addHouseToObserve/' + id)
            .then(function (response) {
                $scope.loadAllHouses();
            });
    }
